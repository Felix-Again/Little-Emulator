...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        while (isRunning)
        {
            ...
            while (!clock->frameComplete())
            {
                uint32_t startCycles = clock->getCycles();
                cpu->executeASM();
                uint32_t elapsedCycles = clock->getCycles() - startCycles;
                gpu->step(elapsedCycles, pixelBuffer);
